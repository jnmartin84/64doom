...
static uint16_t *qarray;//[32768];
static uint32_t qsize;

static void node_enqueue(uint16_t val)
{
    if (qsize == 0) {
        qarray[0] = val;
    }
...
static uint16_t node_dequeue(void)
{
...
static node_t   *new_nodes;
static uint16_t *rewrites;
static int       numNodesExplored;
static uint16_t  node_index;
...
void node_reorder(int numn, node_t *np)
{
    ...
    numNodesExplored-=1;
    node_enqueue(node_index);

    while (qsize > 0) 
    {
        node_index = node_dequeue();
