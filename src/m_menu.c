...
//	}
        ...
        for(int c=0;c<16;c++)
        {
            if(gameid_string[c] == '.') break;
            tmp_string[c] = gameid_string[c];
            length += 1;
        }
...
//char                    savegamestrings[10][SAVESTRINGSIZE]
        ...
        {
             savegamestrings[0][c] = tmp_string[c];
        }
//	close (handle);
	LoadMenu[i].status = 1;
    }
...
void M_DrawLoad(void)
{
    ...
    V_DrawPatch (72,28,W_CacheLumpName("M_LOADG",PU_CACHE));
    for (i = 0;i < load_end; i++)
    {
...
void M_DrawSaveLoadBorder(int x,int y)
{
    ...
    V_DrawPatch (x-8,y+7,W_CacheLumpName("M_LSLEFT",PU_CACHE));

    for (i = 0;i < 24;i++)
    {
	V_DrawPatch (x,y+7,W_CacheLumpName("M_LSCNTR",PU_CACHE));
	x += 8;
    }

    V_DrawPatch (x,y+7,W_CacheLumpName("M_LSRGHT",PU_CACHE));
}
...
void M_DrawSave(void)
{
    ...
    V_DrawPatch (72,28,W_CacheLumpName("M_SAVEG",PU_CACHE));
    for (i = 0;i < load_end; i++)
    {
	M_DrawSaveLoadBorder(LoadDef.x,LoadDef.y+LINEHEIGHT*i);
	M_WriteText(LoadDef.x,LoadDef.y+LINEHEIGHT*i,savegamestrings[i]);
...
void M_DrawSound(void)
{
    V_DrawPatch (60,38,W_CacheLumpName("M_SVOL",PU_CACHE));
...
void M_DrawNewGame(void)
{
    V_DrawPatch (96,14,W_CacheLumpName("M_NEWG",PU_CACHE));
    V_DrawPatch (54,38,W_CacheLumpName("M_SKILL",PU_CACHE));
}
...
void M_DrawEpisode(void)
{
    V_DrawPatch (54,38,W_CacheLumpName("M_EPISOD",PU_CACHE));
}
...
void M_DrawOptions(void)
{
    V_DrawPatch (108,15,W_CacheLumpName("M_OPTTTL",PU_CACHE));
  ...
  int	thermDot )
{
    ...
    xx = x;
    V_DrawPatch (xx,y,W_CacheLumpName("M_THERML",PU_CACHE));
    xx += 8;
    for (i=0;i<thermWidth;i++)
    {
	V_DrawPatch (xx,y,W_CacheLumpName("M_THERMM",PU_CACHE));
	xx += 8;
    }
    V_DrawPatch (xx,y,W_CacheLumpName("M_THERMR",PU_CACHE));
