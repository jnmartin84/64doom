...
void F_TextWrite (void)
{
	...
	for(i=0;i<64;i++)
	{
		...
		#define dx (((x<<6)+i)<<1)
		#define dy (((y<<1)+(SCREENWIDTH>>4))*SCREENWIDTH)
		
		dest32 = (uint32_t *)((uintptr_t)__safe_buffer[_dc-1] + (( (dx)+(dy) )<<1));
		*dest32 = spot;
	...
	for(i=0;i<64;i++)
	{
		uint16_t spot = palarray[*(src+((y&63)<<6)+i)];
		#define dx (((x<<6)+i))
		#define dy (((y+(SCREENWIDTH>>4)))*SCREENWIDTH)

		dest16 = (uint16_t *)((uintptr_t)__safe_buffer[_dc-1] + (( (dx)+(dy) )<<1));
		*dest16 = spot;	
	}
	}
	}
...
uint16_t *dest16;
	...
	while (count--)
	{
		...
		#define cy2 (((cy<<1)+(SCREENWIDTH>>4))*SCREENWIDTH)

		dest32 = (uint32_t *)((uintptr_t)__safe_buffer[_dc-1] + (((cx)+(cy2) )<<1));
		*dest32 = spot;
		dest32+=(SCREENWIDTH>>1);
		*dest32 = spot;
#endif
		...
		#define cx (x)
		#define cy (0 + column->topdelta + (not_count - count))
		#define cy2 ((cy+(SCREENWIDTH>>4))*SCREENWIDTH)
		dest16 = (uint16_t *)((uintptr_t)__safe_buffer[_dc-1]+ (((cx)+(cy2))<<1));
		*dest16 = spot;
#endif
		}
...
// this is where the normal resolution code lives
...
static inline void F_bgpart(void) {
    ...
    for (y=0 ; y<SCREENHEIGHT ; y++)
    {
        ...
        {
            *(uint32_t*)(&dest16[i+sy]) = palarray[src[asy+i]];
            *(uint32_t*)(&dest16[i+64+sy]) = palarray[src[asy+i]];
            *(uint32_t*)(&dest16[i+128+sy]) = palarray[src[asy+i]];
            *(uint32_t*)(&dest16[i+192+sy]) = palarray[src[asy+i]];
            *(uint32_t*)(&dest16[i+256+sy]) = palarray[src[asy+i]];
        }
    }
}
  ...
  int		col )
{
    ...
    while (column->topdelta != 0xff )
    {
        ...
        dest16 = (uint16_t *)((uintptr_t)__safe_buffer[_dc-1] + ((ytab((SCREENWIDTH>>4)+(column->topdelta + not_count)))<<1));
