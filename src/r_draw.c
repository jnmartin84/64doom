...
{ 
    ...
    dest = (uint8_t*)(((uint8_t *)bufptr) + ((viewwindowy + sy)*SCREENWIDTH) + (viewwindowx + sx1));
...
{ 
    ...
    dest = (uint16_t*)(((uint8_t *)bufptr) + ((viewwindowy + sy)*SCREENWIDTH) + viewwindowx + (sx1<<1));
	...
	uint16_t widespot = (spotspot << 8) | spot;
