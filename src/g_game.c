...
void G_BuildTiccmd (ticcmd_t* cmd)
{
    ...
    n64_memcpy (cmd,base,sizeof(*cmd));
...
void G_DoLoadLevel(void)
{
	...
	n64_memset (players[i].frags,0,sizeof(players[i].frags));
    }
    ...
    n64_memset (gamekeydown, 0, sizeof(gamekeydown));
    joyxmove = joyymove = 0;
    mousex = mousey = 0;
    sendpause = sendsave = paused = false;
    n64_memset (mousebuttons, 0, sizeof(mousebuttons));
    n64_memset (joybuttons, 0, sizeof(joybuttons));
}
...
void G_Ticker (void)
{
	...
	if (playeringame[i])
	{
		...
		{
		    char ermac[256];
		    sprintf(ermac, "consistency failure (%i should be %i)", cmd->consistancy, consistancy[i][buf]);
		    I_Error(ermac);
		}
		if (players[i].mo)
		{
...
void G_PlayerFinishLevel (int player)
{
    ...
    n64_memset (p->powers, 0, sizeof (p->powers));
    n64_memset (p->cards, 0, sizeof (p->cards));
...
void G_PlayerReborn (int player)
{
    ...
    n64_memcpy (frags,players[player].frags,sizeof(frags));
    killcount = players[player].killcount;
    itemcount = players[player].itemcount;
    secretcount = players[player].secretcount;

    p = &players[player];
    n64_memset (p, 0, sizeof(*p));

    n64_memcpy (players[player].frags, frags, sizeof(players[player].frags));
...
void G_DeathMatchSpawnPlayer(int playernum)
{
    ...
    {
	char ermac[256];
	sprintf(ermac, "Only %i deathmatch spots, 4 required", selections);
	I_Error(ermac);
    }
...
//#endif
	...
	n64_memcpy (wminfo.plyr[i].frags, players[i].frags
    ...
    if (statcopy)
	n64_memcpy (statcopy, &wminfo, sizeof(wminfo));
...
void G_DoLoadGame(void)
{
    ...
    byte readbuffer[MEMPAK_BLOCK_SIZE * 128];
...
//g_game.c:1321: error: dereferencing type-punned pointer will break strict-aliasing rules
    ...
    byte *to_decode = malloc(dec_size);
    ...
    n64_memset(vcheck,0,sizeof(vcheck));
    sprintf(vcheck,"version %i",VERSION);

//g_game.c: In function 'G_DoLoadGame':
//g_game.c:1244: error: pointer targets in passing argument 1 of 'strcmp' differ in signedness
...
void G_DoSaveGame(void)
{
    ...
    n64_memcpy (save_p, description, SAVESTRINGSIZE);
    save_p += SAVESTRINGSIZE;
    n64_memset (name2,0,sizeof(name2));
    sprintf (name2,"version %i",VERSION);
    n64_memcpy (save_p, name2, VERSIONSIZE);
    save_p += VERSIONSIZE;
...
//    M_WriteFile (name, savebuffer, length);
    ...
    // CHECK HOW MUCH FREE SPACE IS ON THE MEMPAK IN FIRST CONTROLLER
    // THIS IS IN BLOCKS (so 256 byte chunks)
    int available_free_blocks = get_mempak_free_space(0);

    byte *in = _new(char, length);
    n64_memcpy(in, savebuffer, length);
    _setsize(in, length);
    byte *enc = lzw_encode(in, 9);
    last_save_size = _len(enc);

    // size for the compressed data, rounded up to next whole block, plus one for the size block
    ...
    byte size_block[MEMPAK_BLOCK_SIZE] = {0};
    byte final_block[MEMPAK_BLOCK_SIZE] = {0};
...
//    entry_structure_t *tmp_struct = NULL;
    ...
    if (err)
    {
        msg = "Error saving, all data lost.";
        goto the_end_of_saving;
    }
...
the_end_of_saving:
    ...
    players[consoleplayer].message = msg;//"Saved game to mempak.";//GGSAVED;
...
boolean G_CheckDemoStatus(void)
{
	...
	endtime = I_GetTime ();
	char ermac[256];
	sprintf(ermac, "timed %i gametics in %i realtics", gametic, endtime-starttime);
	I_Error(ermac);
    }
	...
	*demo_p++ = DEMOMARKER;
	...
	demorecording = false;
	char ermac[256];
	sprintf(ermac, "Demo %s recorded",demoname);
	I_Error(ermac);
    }
