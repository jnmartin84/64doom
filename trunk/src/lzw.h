...
{
	size_t *x = malloc(sizeof(size_t)*2 + n_item * item_size);
        memset(x, 0x00, sizeof(size_t)*2 + n_item * item_size);
	x[0] = item_size;
	x[1] = n_item;
	return x + 2;
}
...
byte* lzw_decode(byte *in)
{
	...
	if (code != M_EOD) I_Error("Bits did not end in EOD\n");
