...
//    I_InitGraphics ();
    ...
    while (1)
    {
		...
		double colratio = ((double)lastcolcount) / ((double)2000.0);
	    double spanratio = ((double)lastspancount) / ((double)2000.0);
	    double fullratio = (((double)lastcolcount)+((double)lastspancount)) / ((double)4000.0);
        rdp_set_primitive_color(RED_COL);
        rdp_draw_filled_rectangle(0, 0, 319*colratio, 16);
        rdp_set_primitive_color(GREEN_COL);
        rdp_draw_filled_rectangle(0, 16, 319*spanratio, 32);
        rdp_set_primitive_color(BLUE_COL);
        rdp_draw_filled_rectangle(0, 32, 319*fullratio, 48);
        rdp_detach_display();
#endif
        unlockVideo(_dc);
    }
...
#if 1
        uint16_t *video_ptr = &((uint16_t *)__safe_buffer[(_dc)-1])[0]; // start at 20 320-pixel lines into the framebuffer
...
#endif
#if 0
        uint32_t *video_ptr = &((uint32_t *)__safe_buffer[(_dc)-1])[0]; // start at 20 320-pixel lines into the framebuffer
